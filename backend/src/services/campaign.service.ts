...
export async function createCampaign(data: Partial<Campaign>): Promise<Campaign> {
  const result = await query(
    `INSERT INTO campaigns (name, description, subdomain, skin_id, node_set_id, target_states, target_value, require_all, sets_state, max_winners, reward_description, reward_nft_mint, is_active, expires_at, created_by)
...
export async function updateCampaign(id: string, data: Partial<Campaign>): Promise<Campaign | null> {
  ...
  const allowedFields = ['name', 'description', 'subdomain', 'skin_id', 'node_set_id', 'target_states', 'target_value', 'require_all', 'sets_state', 'max_winners', 'reward_description', 'reward_nft_mint', 'is_active', 'expires_at'];
